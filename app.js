...
for (let i = 1; i <= 4; i++) {
    router.get(`/fresh/${i}`, async(ctx)=> {
        let context = {
            ...sider,
            ...company
        };
        await ctx.render(`fresh/fresh${i}`, context);
    });
}

for (let i = 1; i <= 4; i++) {
    router.get(`/faddish/${i}`, async(ctx)=> {
        let context = {
            ...sider,
            ...company
        };
        await ctx.render(`faddish/faddish${i}`, context);
    });
}
...
router.post('/createFresh', async(ctx)=> {
    let context = `{% extends "../fresh.html" %}{% block fresh %}{{ ${ctx.request.body.freshName} }}{% endblock %}`
    let buf = new Buffer(context);
    try {
        await fs.writeFile(path.join(__dirname, `/views/fresh/${ctx.request.body.fileName}.html`), buf, (err) => {
            if (err) throw err;
        });
        ctx.body = {
            status: 200,
            info: 'success to create file'
        }
    } catch (err) {
